class ProductionDataController < ApplicationController
  ...
  def create
    ...
    if @production_datum_new.save
      flash[:notice] = "#{@production_datum.product.number} 制作工程の登録が完了しました。"
      redirect_to product_path(params[:product_id])
    else
      flash[:alert] = "#{@production_datum.product.number} 制作工程の登録に失敗しました。全ての必須項目を入力してください。"
  ...
  def update
    ...
    if @production_datum.update(production_datum_params)
      flash[:notice] = "#{@production_datum.product.number} 制作工程の変更が完了しました。"
      redirect_to product_path(params[:product_id])
    else
      flash[:alert] = "#{@production_datum.product.number} 制作工程の変更に失敗しました。全ての必須項目を入力してください。"
      redirect_back(fallback_location: root_path)
    end
  end
